...
nb_iter_finish=0
save(nb_iter_finish,file="Nb_iter_Test_cvMCMC_p2000.Rdata")
...
Res_Tot<-foreach(s = 1:S, .packages = c("nlme","mvnfast","doParallel","ggplot2","cowplot","glmnet","microbenchmark","tidyverse","nimble")) %dopar% {
  ...
  mcmc_out=FitOneVariableParamSpikeAndSlabDelay_marginal_delta_slow(data = data_input, nu_0 = nu0, niter = 5000)

  load("Nb_iter_Test_cvMCMC_p2000.Rdata")
...
save(Res_Tot,file="Res_Test_cvMCMC_p2000_2.Rdata")

#load("Res_Test_cvMCMC_p2000.Rdata") #Gamma2_init=5000

load("Res_Test_cvMCMC_p2000_2.Rdata") #Gamma2_init=5000
mcmc_out=Res_Tot[[5]]
